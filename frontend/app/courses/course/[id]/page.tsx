          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 sticky top-24 border border-gray-200">
              ...
              <div className="text-center mb-6">
                ...
                {course.tutor.tutor_profile?.title && (
                ...
                {(course.tutor.tutor_profile?.years_of_experience || course.tutor.tutor_profile?.highest_education) && (
                  <div className="text-left mb-4 space-y-2">
                    {course.tutor.tutor_profile.years_of_experience && (
                      <div className="flex items-center space-x-2">
                        <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span className="text-xs text-gray-600">
                          {course.tutor.tutor_profile.years_of_experience} years experience
                        </span>
                      </div>
                    )}
                    {course.tutor.tutor_profile.highest_education && (
                      <div className="flex items-center space-x-2">
                        <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5z" />
                        </svg>
                        <span className="text-xs text-gray-600">
                          {course.tutor.tutor_profile.highest_education.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                        </span>
                      </div>
                    )}
                  </div>
                ...
                {course.tutor.tutor_profile?.specializations && (
                  <div className="text-left mb-4">
                    <h4 className="text-sm font-sora font-semibold text-text-primary mb-2">Specializations</h4>
                    <div className="flex flex-wrap gap-1">
                      {course.tutor.tutor_profile.specializations.split(',').slice(0, 3).map((spec, index) => (
                        <span key={index} className="px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-full">
                          {spec.trim()}
                        </span>
                      ))}
                      {course.tutor.tutor_profile.specializations.split(',').length > 3 && (
                        <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">
                          +{course.tutor.tutor_profile.specializations.split(',').length - 3} more
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {/* Social Links */}
                {(course.tutor.tutor_profile?.linkedin_url || course.tutor.tutor_profile?.github_url || course.tutor.tutor_profile?.portfolio_url) && (
                  <div className="flex justify-center space-x-3">
                    {course.tutor.tutor_profile.linkedin_url && (
                      <a href={course.tutor.tutor_profile.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800">
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                        </svg>
                      </a>
                    )}
                    {course.tutor.tutor_profile.github_url && (
                      <a href={course.tutor.tutor_profile.github_url} target="_blank" rel="noopener noreferrer" className="text-gray-600 hover:text-gray-800">
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    {course.tutor.tutor_profile.portfolio_url && (
                      <a href={course.tutor.tutor_profile.portfolio_url} target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:text-purple-800">
