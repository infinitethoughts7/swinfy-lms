...
const LiveSessionCard = ({
  ...
  return (
    <Card className="overflow-hidden hover:shadow-md transition-all duration-200">
      <CardHeader className="pb-4">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <Badge variant={getStatusVariant(session.status)} className={`capitalize ${session.status === 'live' ? 'animate-pulse' : ''}`}>
              {session.status}
            </Badge>
            {session.status === 'live' && (
      ...
      <CardFooter className="bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
